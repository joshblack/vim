...
function! joshblack#defer#defer(evalable) abort
  if has('autocmd') && has('vim_starting')
    ...
    execute 'autocmd User JBDefer ' . a:evalable
  else
    execute a:evalable
  endif
endfunction
...
function! joshblack#defer#packadd(pack, plugin) abort
  execute "call joshblack#defer#defer('call joshblack#plugin#packadd(\"' . a:pack . '\", \"' . a:plugin . '\")')"
endfunction