" Switch to plaintext mode with: call joshblack#functions#plaintext()
function! joshblack#functions#plaintext() abort
  if has('conceal')
...
function! joshblack#functions#keynote() abort
  if has('gui')
    setlocal nonumber
    ...
    echoerr 'joshblack#functions#keynote() should be run from within a GUI instance of Vim'
  endif
endfunction

function! joshblack#functions#substitute(pattern, replacement, flags) abort
  let l:number=1
...
"
" See also: https://joshblack.com/wiki/Sorting_.gitmodules_entries_with_Vim
function! joshblack#functions#sortgitmodules(...) abort
  if &ft ==# 'gitconfig' || a:0
    silent %s/\v\n\t/@@@/e
    %sort
    silent %s/\v\@\@\@/\r\t/ge
  else
    echomsg 'Not a "gitconfig" file: use `joshblack#functions#sortgitmodules(1)` to force'
  endif
endfunction

" http://stackoverflow.com/a/39348498/2103996
function! joshblack#functions#clearregisters() abort
...
function! joshblack#functions#echoerr(msg) abort
  try
    echohl ErrorMsg
    echomsg a:msg
  finally
