...
function! joshblack#commands#preview(...) abort
  if a:0 == 0
...
function! s:open_in_diffusion(file) abort
  ...
  if l:relative_path ==# ''
    call joshblack#functions#echoerr('Could not find repo configuration for file ' . a:file)
    return
  endif
...
function! joshblack#commands#open_in_diffusion(...) abort
  if a:0 == 0
    let l:files=[expand('%')]
  else
    let l:files=a:000
  ...
  if !l:did_open
    call joshblack#functions#echoerr('No filename')
    return
  endif
endfunction
...
function! joshblack#commands#search(query) abort
  if a:query ==# ''
